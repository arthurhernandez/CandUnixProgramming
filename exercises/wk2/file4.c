...
int main()
{
   ...
   if(birthMo >= HIGHMO){ printf( "Invalid month\n"); }
   else if(birthMo == 2 || birthMo == 3){
      if(birthYr%4  == 0 && birthYr%10 != 0 || birthYr%400 == 0){
         if(birthDay <= HIDAY1){ printf( "Valid day\n"); }
         else{ printf( "Invalid day for month %d in %d\n", birthMo, birthYr);}
      }else{
	 if(birthDay > HIDAY1){ printf( "Valid day\n");}
         else{ printf( "Invalid day for month %d in %d\n", birthMo, birthYr ); }
   }
   if(birthMo < 8 && birthMo%2 == 1 | birthMo < 7 && birthMo%2 != 0){
      if(birthDay >= HIDAY1){
        printf( "Valid day"  );
      }else{
