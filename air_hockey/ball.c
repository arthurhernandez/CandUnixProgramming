...
//check if the ball collides with the top goal area 
bool checkCollisionTopGoalZone(ball_t *b, zone_t *z){
      if (b->upper_left_x<=z->upper_left_x || b->upper_left_x>=z->upper_left_x + z->width){
        b->speed_x *= -1;
        return true;
      }
...
//check if the ball collides with the Bottom  goal area
bool checkCollisionBottomGoalZone(ball_t *b, zone_t *z){
    if (b->upper_left_x<=z->upper_left_x || b->upper_left_x>=z->upper_left_x + z->width){
        b->speed_x *= -1;
        // b->speed_y *= -1;
